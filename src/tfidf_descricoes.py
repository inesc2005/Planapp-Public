"""
TF-IDF com Descrições de Categorias (Zero-Shot)
================================================
Classifica as 240 notícias do golden set usando TF-IDF treinado
em descrições manuais das categorias — sem ver os dados do golden set.

Comparável ao LLM: ambos usam apenas conhecimento prévio (descrições/prompt).

Output:
  data/evaluation/preds_tfidf.json
...
OUT_FILE     = OUT_DIR / "preds_tfidf.json"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# =============================================================================
# DESCRIÇÕES MANUAIS POR CATEGORIA (zero-shot — nunca vê o golden set)
# Cada descrição é um conjunto de palavras-chave representativas da categoria.
# =============================================================================
DESCRICOES = {
    ...
    "habitação": (
        "habitação habitação arrendamento imobiliário casa alojamento renda preço "
    ...
    "transportes": (
        ...
        "transporte público carros elétricos bicicleta bicicleta partilhada "
    ...
    "demografia": (
        ...
        "imigração natalidade taxa fertilidade esperança vida censos "
...
def classificar_tfidf_descricoes(df_gold: pd.DataFrame) -> dict:
    """
    Para cada notícia, computa a similaridade cosseno com cada descrição
    e normaliza para obter pseudo-probabilidades.

    Retorna dict: { str(id): {"categoria": str, "probs": {cat: float}} }
    """
    print("A vectorizar descrições e notícias com TF-IDF...")
    ...
    vectorizer = TfidfVectorizer(
        max_features=15000,
        ngram_range=(1, 2),
        sublinear_tf=True,  # aplica log(TF) — melhora para textos longos
    )

    # Treina nos descrições + notícias para vocabulário comum
    todos_textos = descricoes_lista + noticias_lista
    vectorizer.fit(todos_textos)

    tfidf_descricoes = vectorizer.transform(descricoes_lista)  # (24, vocab)
    tfidf_noticias   = vectorizer.transform(noticias_lista)    # (240, vocab)

    # Similaridade cosseno: (240, 24)
    sims = cosine_similarity(tfidf_noticias, tfidf_descricoes)

    # Normalizar cada linha para somar 1 → pseudo-probabilidades
    row_sums = sims.sum(axis=1, keepdims=True)
    row_sums[row_sums == 0] = 1  # evitar divisão por zero
    probs_matrix = sims / row_sums  # (240, 24)
...
def main():
    ...
    resultados = classificar_tfidf_descricoes(df_gold)
